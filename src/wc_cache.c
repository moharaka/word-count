...
int wcc_init(uint32_t nb_thread, uint32_t nb_word)
{
    ...
    for (i = 0; i < nb_thread; i++) {
        if (ht_init(&thread_wc_caches[i].htable, wcc_hash_bkt, wcc_compare, nb_buckets))
            return -1;
    }

    if (ht_init(&main_wc_cache.htable, wcc_hash_bkt, wcc_compare_main, nb_buckets))
        return -1;
...
static uint32_t get_code(char *word)
{
    ...
    shift = (char)(sizeof(unsigned int) * CHAR_BIT - __builtin_clz(NB_LETTERS));
...
static uint32_t __hash(unsigned char *str)
{
    uint32_t hash = 5381;
    int c;
...
static int __wcc_merge_results(uint32_t tid, uint32_t bckt, struct wc_cache_s *wcc);
...
//#if 1
    printf("Total word %d, total words counter %d, full bkt %d (ideal %d)\n",
           total, count_total, nb_buckets - empty_bkt, MIN(total, nb_buckets));
#endif
    return 0;
}
